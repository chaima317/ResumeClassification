...
/* creation d"une table historoique CVs qui permet d'enregistrer l'historique des cv de la meme personne 
-Si le Cv contient le meme mail ==> on enregistre l'id du CV avec la date du jour et un numéro incremental qui 
correspond a la version du cvs

*/

Create TABLE historique_CVS(
   IDCV VARCHAR2(150), 
   DATE_ DATE,
   VERSIONS NUMBER,
   CONSTRAINT PK_CV_HISTORIQUE PRIMARY KEY(IDCV,VERSIONS),
   CONSTRAINT FK_CV_HISTORIQUE FOREIGN KEY (IDCV) REFERENCES CV(IDCV)
 
);
/*CREATION D'UNE SEQUENCE POUR LES VERSION*/
...
/*
CERATION D"UN TRIGGER QUI PERMET DE REMPLIR LA TABLE HISTORIQUE CV APRES CHAQUE INSERTION DE CANDIDATS AVEC LE MEME MAIL

*/


CREATE OR REPLACE TRIGGER AFTER UPDATE OF MAILCAN ON CANDIDATS
FOR EACH ROW 

BEGIN 

IF (:new.MAILCAN==:OLD.MAILCAN)

THEN 
  EXECUTE IMMEDIATE 'INSERT INTO historique_CVS(IDCV,DATE_,VERSION) VALUES (' || CHR(39) || CANDIDATS.IDCV || CHR(39) ||',' || CHR(39) || SYSDATE|| CHR(39) ||','|| CHR(39) || dept_seq.NEXTVAL || CHR(39) ||')';
